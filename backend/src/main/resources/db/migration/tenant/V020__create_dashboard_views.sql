...
-- =====================================================
-- Drop all views and functions first (in dependency order)
-- =====================================================
DROP FUNCTION IF EXISTS get_dashboard_snapshot() CASCADE;
DROP VIEW IF EXISTS v_dashboard_summary CASCADE;
DROP VIEW IF EXISTS v_pending_purchase_orders CASCADE;
DROP VIEW IF EXISTS v_top_products_today CASCADE;
DROP VIEW IF EXISTS v_sales_trend_7days CASCADE;
DROP VIEW IF EXISTS v_pending_orders_summary CASCADE;
DROP VIEW IF EXISTS v_critical_stock_products CASCADE;
DROP VIEW IF EXISTS v_inventory_value_summary CASCADE;
DROP VIEW IF EXISTS v_daily_sales_by_channel CASCADE;

-- =====================================================
-- View 1: Daily Sales Summary
-- =====================================================
-- Shows sales summary for current day
CREATE VIEW v_daily_sales_by_channel AS
SELECT
    o.status,
...
COMMENT ON VIEW v_daily_sales_by_channel IS 'Daily sales summary by status';
...
SELECT
    ...
    SUM(
        COALESCE(i.available_quantity, 0) * COALESCE(pc.average_cost, 0)
...
SELECT
    ...
    COALESCE(pc.average_cost, 0) * (i.min_quantity - i.available_quantity) AS replenishment_cost,
...
-- =====================================================
-- Shows summary of orders pending fulfillment by status
CREATE VIEW v_pending_orders_summary AS
SELECT
    o.status,
    COUNT(DISTINCT o.id) AS order_count,
    ...
    COUNT(
        CASE
            WHEN o.created_at < CURRENT_TIMESTAMP - INTERVAL '2 days' THEN 1
        END
    ) AS overdue_count
FROM orders o
...
COMMENT ON VIEW v_pending_orders_summary IS 'Pending orders requiring action grouped by status';
...
CREATE VIEW v_sales_trend_7days AS
SELECT
...
SELECT
    ...
    CURRENT_DATE - DATE(po.created_at) AS days_pending,
    CASE
...
GROUP BY po.id, po.po_number, po.status, s.company_name, s.supplier_code,
         po.created_at, po.expected_delivery_date, po.total
ORDER BY
    CASE po.status
...
CREATE INDEX IF NOT EXISTS idx_inventory_available_qty
    ON inventory(location_id, product_id, available_quantity)
    WHERE available_quantity > 0;

-- Index for critical stock queries
CREATE INDEX IF NOT EXISTS idx_inventory_minimum_check
    ON inventory(location_id, product_id, available_quantity, min_quantity)
