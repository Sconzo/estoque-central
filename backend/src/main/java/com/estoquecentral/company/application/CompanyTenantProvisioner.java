...
/**
 * CompanyTenantProvisioner - Service for provisioning new tenant schemas during company creation.
 *
 ...
 * </ul>
 *
 * <p><strong>Note:</strong> Renamed from TenantProvisioner to CompanyTenantProvisioner
 * to avoid Spring bean conflict with com.estoquecentral.tenant.application.TenantProvisioner
 *
 * @since 1.0
 */
@Service
public class CompanyTenantProvisioner {

    private static final Logger logger = LoggerFactory.getLogger(CompanyTenantProvisioner.class);

    private final DataSource dataSource;
    private final JdbcTemplate jdbcTemplate;

    public CompanyTenantProvisioner(DataSource dataSource, JdbcTemplate jdbcTemplate) {
     ...
     *
     * <p><strong>IMPORTANT:</strong> NO @Transactional here! Flyway manages its own transactions.
     * Having @Transactional(REQUIRES_NEW) causes deadlock when Flyway tries to commit migrations
     * because Spring holds an outer transaction that never commits while Flyway waits.
     *
